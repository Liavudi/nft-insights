...
# TODO 1. Built the chart data you want to use
# TODO 2. upload the chart data to elastic search
# TODO 3. From elastic search fetch the chart data and show it in the chart

def populate_one_day_to_elastic(starting_timestamp: int):
    one_hour = 3600
    eth_to_usd_list = []
    for i in range(3):
        eth_to_usd_list.append({"timeStamp": starting_timestamp,"price": get_average_contracts_gas_fee('0xb47e3cd837dDF8e4c57F05d70Ab865de6e193BBB',starting_timestamp, starting_timestamp + one_hour)}) 
        starting_timestamp += one_hour
    # jsonDataTest(eth_to_usd_list)
    es_client.populate_eth_to_usd_index(eth_to_usd_list)

# populate_one_day_to_elastic(1663981200)
